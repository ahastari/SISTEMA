{{-- ============================================ --}}
{{-- MODALES DE USUARIO - COMPATIBLES CON TEMA OSCURO Y CLARO --}}
{{-- ============================================ --}}
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <form action="{{ route('configuracion.usuarios.password', $user->id) }}" method="POST">
                ...
                <div class="modal-header bg-body-tertiary border-bottom py-3">
                    <h6 class="modal-title fw-bold text-body"><i class="bi bi-key text-primary me-2"></i>Nueva Contraseña</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-3 bg-body">
                    <label class="form-label small text-body-secondary">Establecer clave para: <strong class="text-body">{{ $user->name }}</strong></label>
                    <input type="password" name="password" class="form-control rounded-3 bg-body-tertiary text-body border" placeholder="Mínimo 6 caracteres" required minlength="6">
                </div>
                <div class="modal-footer border-top p-2 bg-body-tertiary">
...
<div class="modal fade" id="modalEditarUsuario{{ $user->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header bg-body-tertiary border-bottom py-3">
                <h5 class="modal-title fw-bold text-body"><i class="bi bi-person-gear text-primary me-2"></i>Modificar Perfil Operador</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('configuracion.usuarios.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body p-4 bg-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control rounded-3 bg-body-tertiary text-body border" value="{{ $user->name }}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-semibold text-body-secondary">Correo Electrónico</label>
                        <input type="email" name="email" class="form-control rounded-3 bg-body-tertiary text-body border" value="{{ $user->email }}" required>
                    </div>
                    <div class="row g-2 mb-3">
                        <div class="col-6">
                            <label class="form-label small fw-semibold text-body-secondary">Rol Autorizado</label>
                            <select name="role" class="form-select rounded-3 bg-body-tertiary text-body border" required>
                ...
                <div class="modal-footer border-top p-3 bg-body-tertiary">
                    <button type="button" class="btn btn-outline-secondary btn-sm rounded-3 px-3" data-bs-dismiss="modal">Cancelar</button>
